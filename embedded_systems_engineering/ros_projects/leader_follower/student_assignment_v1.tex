...
\midrule
ITEMIS Create  & Graphical statechart editor      & 2 \\
...
The following folder structure is a proposal for organising your project files.:

\begin{lstlisting}[style=shellcode]
leader_follower/
  models/
    statecharts/
      leader_statechart.ysc       # Leader ITEMIS Create model
      follower_statechart.ysc     # Follower ITEMIS Create model
    uppaal/
      leader_follower.xml         # Uppaal model 
      leader_follower.q           # Verification queries
  src/
    leader_follower_msgs/         # Custom ROS2 messages 
    ...
    follower_node/
      follower_node/
        follower_node.py          
    visualization_node/           # visualises robots in RViz2
\end{lstlisting}
...
% \end{enumerate}
% \end{Ex}
\begin{Ex}{Statechart Creation}
\begin{enumerate}
  \item Model the leader and follower behaviours as statecharts in ITEMIS Create, following
        the provided requirements and edge cases.
  \item Extend your statecharts to cover a scenario not yet modelled (e.g.\ battery low, sensor fault).
        Justify your choice.
...
A \textbf{network of timed automata} (NTA) is a set of TA running concurrently, with
...
Here are examples of global channel declarations you might find useful when designing your own model. The list is not exhaustive; you may need to add more channels for additional scenarios or edge cases.
...
\begin{enumerate}
  \item Model the leader and follower behaviours as timed automata in Uppaal, following the provided requirements and edge cases.
  \item  Run all 14 queries below. Record your results in the following table (your model needs to take care of the mentioned signals and timeouts for the queries to pass):
\end{enumerate}
...
\begin{enumerate}
  ...
  \item A failure can be modelled by an explicit \texttt{ERROR} state.
        Add an \texttt{ERROR} state to both the \texttt{Leader} and \texttt{Follower} templates and modify query 12 to \emph{assume} neither robot is in \texttt{ERROR}
        and re-verify. 
...
\begin{enumerate}
  \item Model distance changes
        by cycling through values 0–2 deterministically in an \texttt{Environment} automaton.
  \item Modify it so that the distance can also jump directly to a LOST state
        at any time, simulating sudden signal loss.
        Re-verify the deadlock-freedom query. 
...
\begin{infobox}{Goal of this Stage}
Consolidate your verification results and argue formally about key system properties in your presentation.
\end{infobox}

\begin{Ex}{Verification Report}
For the presentation, prepare a concise report summarising your verification results.
\begin{enumerate}
...
The system can run as five cooperating ROS2 nodes:
...
\begin{tikzpicture}[
  ...
  %% ── Visualization node at top centre ──
  \node[rosnode] (vn) at (4,7.5) {visualization\\node};

  %% ── Status topics: each near its publisher ──
  \node[topic] (ls) at (2,6)   {/leader/status};
  \node[topic] (fs) at (6,6)   {/follower/status};

  %% ── Main robot nodes ──
  \node[rosnode] (ln) at (0,4)  {leader\_node};
  \node[rosnode] (fn) at (8,4)  {follower\_node};

  %% ── Command topics: on the outer sides ──
  \node[topic] (fc) at (-2.5,2) {/follower/command};
  \node[topic] (lc) at (10.5,2) {/leader/command};

  %% ── Infrastructure nodes at the bottom ──
  \node[rosnode] (slam) at (-2.5,0) {slam\_toolbox\\(leader)};
  \node[rosnode] (nav)  at (10.5,0) {nav2\\(both)};

  %% ── /leader/status ──
  \draw[pub] (ln.north) -- (ls.south west);           % leader publishes
  \draw[sub] (ls.east)  -- (fn.north west);            % follower subscribes
  \draw[sub] (ls.north) -- (vn.south west);            % viz subscribes

  %% ── /follower/status ──
  \draw[pub] (fn.north) -- (fs.south east);            % follower publishes
  \draw[sub] (fs.west)  -- (ln.north east);            % leader subscribes
  \draw[sub] (fs.north) -- (vn.south east);            % viz subscribes

  %% ── /follower/command ──
  \draw[pub] (ln.south west) -- (fc.north);            % leader publishes
  \draw[sub] (fc.east) -- (fn.south west);             % follower subscribes

  %% ── /leader/command ──
  \draw[pub] (fn.south east) -- (lc.north);            % follower publishes
  \draw[sub] (lc.west) -- (ln.south east);             % leader subscribes
\end{tikzpicture}
\caption{ROS2 node communication graph. Green solid = publish; orange dashed = subscribe.}
\end{figure}

\begin{Ex}{Implementing the Nodes}
\begin{enumerate}
  \item Implement the \texttt{leader\_node} and \texttt{follower\_node} state machine logic in Python, following the verified model.
  \item Implement the \texttt{visualization\_node} to display the positions and states of both robots in RViz2 using markers.
  \item Integrate the \texttt{slam\_toolbox} for the leader and \texttt{nav2} for both robots to enable autonomous navigation and obstacle avoidance.
\end{enumerate}
\end{Ex}
...
\begin{enumerate}
  \item Gazebo loads an exploration world (build a sample world); both robots spawn.
  ...
  \item Leader navigates into a dead-end corridor.
...
\begin{enumerate}
  \item Build a simple Gazebo world with an open area and a dead-end corridor.
...
\begin{enumerate}
  ...
  \item Record a video of the full demo (initial approach + exploration +
        at least one edge case recovery). 
\end{enumerate}
\end{Ex}
