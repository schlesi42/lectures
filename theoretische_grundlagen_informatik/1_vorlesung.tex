...
\end{frame}
  ...
  \begin{frame}{Quotientenmenge}
    Sei $R\subseteq M\times M$ eine "Aquivalenzrelation auf der Menge $M$. F"ur $x\in M$ definieren wir die \textbf{"Aquivalenzklasse} von $x$ bez"uglich $R$ als 
    \[[x]_R:=\{y\in M|(x,y)\in R\}\]
    Die Menge aller "Aquivalenzklassen nennen wir \textbf{Quotientenmenge} von $M$ bez"uglich $R$ und schreiben 
    \[M/R:=\{[x]_R|x\in M\}\]
  \end{frame}

\begin{frame}{Partitionen}
\begin{definition}{Partition}
  Eine Mengenfamilie $P\subseteq\mathscr{P}(M)$ f"ur eine Menge $M$ ist eine \textbf{Partition} von $M$, wenn
  \begin{itemize}
  ...
  \end{itemize}
\end{definition}
Also ist eine Partition eine Menge von paarweise disjunkten, nicht-leeren Teilmengen, deren Vereinigung die Gesamtmenge ergibt.
\end{frame}
\begin{frame}{Zusammenhang von Partitionen und "Aquivalenzrelationen}
  \begin{itemize}
    \item Jede "Aquivalenzrelation auf einer Menge $M$ definiert eine Partition von $M$, n"amlich die Quotientenmenge $M/R=\{[x]_R|x\in M\}$ mit $[x]_R=\{y\in M|(x,y)\in R\}$.
    \item Umgekehrt definiert jede Partition einer Menge $M$ eine "Aquivalenzrelation auf $M$. Die kanonische Relation ist \[R=\{(x,y)\in M\times M|\exists A\in P:x\in A\wedge y\in A\}\]
  \end{itemize}
\end{frame}
...
\begin{frame}{Der Homomorphiesatz (Kanonische Zerlegung)}
    Sei $f: M \to N$ eine beliebige Abbildung. Dann existiert eine Zerlegung:
    
    \vspace{0.5cm}
    
    \begin{itemize}
        \item \textbf{"Aquivalenzrelation:} $x \equiv y \iff f(x) = f(y)$
        \item \textbf{Projektion:} $\pi: M \to M/{\equiv}, \quad x \mapsto [x]$
        \item \textbf{Induzierte Abb.:} $\tilde{f}: M/{\equiv} \to N, \quad [x] \mapsto f(x)$
    \end{itemize}

    \vspace{0.5cm}
    
    Das Diagramm kommutiert, d.h. $f = \tilde{f} \circ \pi$. 
    Dabei ist $\pi$ surjektiv und $\tilde{f}$ injektiv.
\end{frame}

\begin{frame}{Beispiel}
  Sei $f:\mathbb{Z}\to\mathbb{Z}, x\mapsto x^2$. Dann ist die zu $f$ kanonische Zerlegung:
  \begin{itemize}
    \item \textbf{"Aquivalenzrelation:} $x\equiv y\iff x^2=y^2\iff x=y\vee x=-y$
    \item \textbf{Projektion:} $\pi:\mathbb{Z}\to\mathbb{Z}/\equiv, x\mapsto [x]$
    \item \textbf{Induzierte Abb.:} $\tilde{f}:\mathbb{Z}/\equiv\to\mathbb{Z}, [x]\mapsto x^2$
  \end{itemize}
\end{frame}
