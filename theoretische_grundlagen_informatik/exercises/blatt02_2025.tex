...
\aufgabetitel{$5$}{Python - Two Sums}\\
Gegeben ist ein Array ganzer Zahlen \texttt{nums} und eine ganze Zahl \texttt{target}. 
Finde die Indizes der beiden Zahlen, deren Summe gleich \texttt{target} ist.

Du darfst dabei annehmen, dass es genau \emph{eine} Lösung gibt, 
und dass dasselbe Element nicht zweimal verwendet werden darf.

Die Reihenfolge der zurückgegebenen Indizes ist beliebig.
...
\begin{enumerate}
  \item 
  \textbf{Eingabe:} \texttt{nums = [2, 7, 11, 15]}, \texttt{target = 9} \\
  \textbf{Ausgabe:} \texttt{[0, 1]} \\
  \textbf{Begründung:} \texttt{nums[0] + nums[1] = 2 + 7 = 9}, daher geben wir \texttt{[0, 1]} zurück.

  \item 
  \textbf{Eingabe:} \texttt{nums = [3, 2, 4]}, \texttt{target = 6} \\
  \textbf{Ausgabe:} \texttt{[1, 2]}

  \item 
  \textbf{Eingabe:} \texttt{nums = [3, 3]}, \texttt{target = 6} \\
  \textbf{Ausgabe:} \texttt{[0, 1]}
\end{enumerate}

\aufgabetitel{$5$}{Python - Duplicates}\\


Gegeben ist ein Array ganzer Zahlen \texttt{nums}.  
Gib \texttt{true} zurück, falls ein Wert im Array mindestens zweimal vorkommt,  
und \texttt{false}, falls alle Elemente verschieden sind.
...
\begin{enumerate}
  \item 
  \textbf{Eingabe:} \texttt{nums = [1, 2, 3, 1]} \\
  \textbf{Ausgabe:} \texttt{true} \\
  \textbf{Begründung:} Das Element \texttt{1} kommt an den Indizes 0 und 3 vor.

  \item 
  \textbf{Eingabe:} \texttt{nums = [1, 2, 3, 4]} \\
  \textbf{Ausgabe:} \texttt{false} \\
  \textbf{Begründung:} Alle Elemente sind verschieden.

  \item 
  \textbf{Eingabe:} \texttt{nums = [1, 1, 1, 3, 3, 4, 3, 2, 4, 2]} \\
  \textbf{Ausgabe:} \texttt{true}
\end{enumerate}
